...
describe('MadPromise', () => {
  ...
  describe('advanced behavior', () => {
    it('should allow multiple then callbacks to the same promise', (done) => {
      const p = MadPromise((resolve) => {
        setTimeout(() => resolve(42), 100);
      });
    ...
    it('should allow multiple catch callbacks to the same promise', (done) => {
      const p = MadPromise((resolve, reject) => {
        setTimeout(() => reject(42), 100);
      });
      ...
      p.catch((v) => (firstResult = v));
      p.catch((v) => (secondResult = v));
    ...
    describe('that promises are deterministic', () => {
      it('should handle then callbacks in a deterministic manner, to prevent race condition bugs', (done) => {
        const log = [];
      ...
      it('should handle catch callbacks in a deterministic manner, to prevent race condition bugs', (done) => {
        const log = [];
    ...
    describe('status can only transition once', () => {
      it('should allow a state transitions to "FULFILLED" to occur once, to prevent strange bugs', (done) => {
        const p = MadPromise((resolve) => {
          resolve(42);
          
          // 1337 should be ignored
          resolve(1337);
        });
    ...
    describe('when then / catch callbacks return promises', () => {
      describe('RESOLVED', () => {
        it('should when return promise is rejectd continue with promise result', (done) => {
          const p = MadPromise((resolve) => {
            setTimeout(() => resolve(42), 100);
          });
...
describe('MadPromise.all', () => {
  ...
  it('should reject when one of the promises is rejected', (done) => {
    ...
    const b = MadPromise((resolve, reject) => {
      setTimeout(() => reject('b'), 80);
    });

    const c = MadPromise((resolve, reject) => {
      setTimeout(() => reject('c'), 90);
    });

    MadPromise.all([a, b, c]).catch((e) => {
      expect(e).toBe('a');

      // Wait for the all to be over before declaring done
